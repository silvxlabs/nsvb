...
pub extern "C" fn stem_wood_volume_ffi(
    ...
    match models::stem_wood_volume(spcd, dia, ht, division_str) {
        Ok(volume) => volume,
        Err(_) => -1.0,
    }
}
...
pub extern "C" fn stem_bark_volume_ffi(
    ...
    match models::stem_bark_volume(spcd, dia, ht, division_str) {
        Ok(volume) => volume,
        Err(_) => -1.0,
    }
}
...
pub extern "C" fn total_biomass_ffi(spcd: u16, dia: f64, ht: f64, division: *const c_char) -> f64 {
    ...
    match models::total_biomass(spcd, dia, ht, division_str) {
        Ok(volume) => volume,
        Err(_) => -1.0,
    }
}
